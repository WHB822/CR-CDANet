...
from diffusion.dadiff import DDPM
...
def main(loop):
    ...
    if resume_training:
        if loop == 0:
            target_model_path = f'{RESULT_ROOT}/source/Islandtown/model/DF_model/sar_rgb/ddpm_model-best.pt'
        elif loop == 1:
            target_model_path = f'{RESULT_ROOT}/Islandtown_shuguang/model/DF_model/sar_rgb_0/ddpm_model-best.pt'
        elif loop == 2:
            target_model_path = f'{RESULT_ROOT}/Islandtown_shuguang/model/DF_model/sar_rgb_1/ddpm_model-best.pt'
        elif loop == 3:
            target_model_path = f'{RESULT_ROOT}/Islandtown_shuguang/model/DF_model/sar_rgb_2/ddpm_model-best.pt'

        source_model_path = f'{RESULT_ROOT}/source/Islandtown/model/DF_model/sar_rgb/ddpm_model-best.pt'
    ...
    for epoch in range(num_epochs):
        ...
        total_loss = 0.0
        target_loss = 0.0
        total_mmd_coral_loss = 0.0
        total_total_mmd_loss = 0.0
        total_total_coral_loss = 0.0
        ...
        for batch_idx, (img_t1, img_t2, label) in enumerate(tqdm(train_loader)):
            ...
            loss, target_loss, mmd_coral_loss, total_mmd_loss, total_coral_loss = ddpm.optimize_parameters(img_t1, img_t2, source_img_t1, source_img_t2)
            ...
            total_total_mmd_loss += total_mmd_loss.item()
            total_coral_loss = torch.tensor(total_coral_loss)
            total_total_coral_loss += total_coral_loss.item()

        avg_loss = total_loss / len(train_loader)
        avg_target_loss = target_loss / len(train_loader)
        avg_mmd_coral_loss = total_mmd_coral_loss / len(train_loader)
        avg_total_mmd_loss = total_total_mmd_loss / len(train_loader)
        avg_total_coral_loss = total_total_coral_loss / len(train_loader)
        ...
        if (epoch + 1) % save_interval == 0:
            save_dir = f'{RESULT_ROOT}/Islandtown_shuguang/model/DF_model/sar_rgb_{loop}/'
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                for batch_idx, (img_t1, img_t2, label) in enumerate(val_loader):
                    ...
                    generated_image = ddpm.test(img_t1, img_t2)

                    psnr_batch = psnr(generated_image.cpu().detach().numpy(), img_t2.cpu().detach().numpy())
                    # 记得改这里！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
                    print(f"当前块psnr值: {psnr_batch:.4f} dB")

                    val_psnr += psnr(generated_image.cpu().detach().numpy(), img_t2.cpu().detach().numpy())
    ...
    # 测试
    # 改这里！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    load_dir = f'{RESULT_ROOT}/Islandtown_shuguang/model/DF_model/sar_rgb_{loop}/ddpm_model-best.pt'
    ddpm.load_network(load_dir)
    ddpm.netG.eval()
    test_psnr = 0.0
    with torch.no_grad():
        for batch_idx, (img_t1, img_t2, label) in enumerate(test_loader):
            ...
            original_patches.extend(img_t2.detach().cpu())
    ...
    # 改这里！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    # 保存为 .mat 文件
    output_dir = f'{RESULT_ROOT}/Islandtown_shuguang/result/DF_result/sar_rgb_{loop}'
    output_path = os.path.join(output_dir, f"{dataset_name}_RGB_ddpm_{loop}.mat")
    output_path_original = os.path.join(output_dir, f"{dataset_name}_RGB_ddpm_original_{loop}.mat")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Training RGB Model")
